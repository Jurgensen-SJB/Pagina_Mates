...
export default function FalsePositionPage() {
  const method = getMethodById('false-position')!;
  const { result, loading, error, warning, execute, clear } = useCalculation('false-position');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    await execute({
      ...
      parameters: {
        ...
        tolerance: parseFloat(tolerance), maxIterations: parseInt(maxIter),
      },
    });
  };
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      ...
      {error && (
        <div className="mb-6 p-4 rounded-lg border animate-slideDown"
          style={{ background: 'rgba(239,68,68,0.1)', borderColor: 'var(--danger)', color: 'var(--danger-light)' }}>
          ❌ {error}
        </div>
      )}
      {warning && !error && (
        <div className="mb-6 p-4 rounded-lg border animate-slideDown"
          style={{ background: 'rgba(245,158,11,0.1)', borderColor: 'var(--warning)', color: 'var(--warning-light)' }}>
          ⚠️ {warning}
        </div>
      )}
          ...
          <div className="glass-card p-6">
            ...
            {showChart && parsedFn.success && parsedFn.fn && (
              <div className="mt-4 space-y-4">
                <FunctionChart fn={parsedFn.fn} xMin={parseFloat(a) - 1} xMax={parseFloat(b) + 1}
