...
export default function FixedPointPage() {
  const method = getMethodById('fixed-point')!;
  const { result, loading, error, warning, execute, clear } = useCalculation('fixed-point');

  const [gFuncStr, setGFuncStr] = useState('(x + 2)^(1/3)');
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      ...
      <form onSubmit={handleSubmit} className="glass-card p-6 mb-8 animate-fadeInUp" style={{ animationDelay: '0.1s' }}>
        ...
        <div className="space-y-4">
          <FunctionInput value={gFuncStr} onChange={setGFuncStr} onError={setFuncError}
            label="g(x) = (la función de iteración, x = g(x))" placeholder="Ej: (x + 2)^(1/3)" />
          <div className="p-3 rounded-lg text-xs" style={{ background: 'rgba(245,158,11,0.1)', color: 'var(--warning-light)' }}>
            💡 <strong>Nota:</strong> Debe transformar f(x)=0 en la forma x = g(x). Por ejemplo, x³ - x - 2 = 0 → x = (x + 2)^(1/3).
            La convergencia requiere |g&apos;(x)| &lt; 1 en la vecindad de la raíz.
          </div>
        ...
        <div className="space-y-6 animate-fadeInUp">
          ...
          <div className="glass-card p-6">
            <label className="flex items-center gap-3 cursor-pointer">
              ...
              <span className="text-sm font-medium" style={{ color: 'var(--text-primary)' }}>📊 Mostrar gráfica de convergencia</span>
            </label>
            {showChart && (
              <div className="mt-4">
                <ConvergenceChart steps={result.steps} tolerance={parseFloat(tolerance)} title="Convergencia" />
              </div>
            )}
          </div>
        </div>
