...
export default function BisectionPage() {
  const method = getMethodById('bisection')!;
  const { result, loading, error, warning, execute, clear } = useCalculation('bisection');
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div>
              ...
              <input type="number" value={maxIter} onChange={e => setMaxIter(e.target.value)}
                className="input-field" required min="1" max="1000" />
            </div>
          </div>
        </div>
            ...
            {showChart && parsedFn.success && parsedFn.fn && (
              <div className="mt-4 space-y-4">
                <FunctionChart
                  ...
                  xMin={parseFloat(a) - 1}
                  xMax={parseFloat(b) + 1}
                  root={result.success ? (result.result as number) : undefined}
