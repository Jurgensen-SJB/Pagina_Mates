...
export default function LagrangePage() {
  const method = getMethodById('lagrange')!;
  const { result, loading, error, warning, execute, clear } = useCalculation('lagrange');
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      <div className="mb-8 animate-fadeInUp">
        ...
        <div className="mt-3 p-3 rounded-lg text-sm" style={{ background: 'var(--surface)', color: 'var(--text-muted)' }}>
          <strong style={{ color: 'var(--text-secondary)' }}>Fórmula:</strong> P(x) = Σ yᵢ·Lᵢ(x), donde Lᵢ(x) = Π (x-xⱼ)/(xᵢ-xⱼ) para j≠i
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="glass-card p-6 mb-8 animate-fadeInUp" style={{ animationDelay: '0.1s' }}>
            ...
            <div key={i} className="flex items-center gap-3">
              ...
              <div className="flex items-center gap-2 flex-1">
                ...
                <input type="number" step="any" value={point.x} onChange={e => updatePoint(i, 'x', e.target.value)}
                  className="input-field flex-1" placeholder="x" required />
                ...
                <input type="number" step="any" value={point.y} onChange={e => updatePoint(i, 'y', e.target.value)}
                  className="input-field flex-1" placeholder="y" required />
              </div>
              {points.length > 2 && (
        ...
        {points.length > 20 && (
          <div className="mt-2 p-2 rounded text-xs" style={{ background: 'rgba(245,158,11,0.1)', color: 'var(--warning-light)' }}>
            ⚠️ Muchos puntos pueden causar el fenómeno de Runge (oscilaciones en los extremos).
          </div>
        )}
        ...
        <div className="mt-4">
          ...
          <input type="text" value={evalAt} onChange={e => setEvalAt(e.target.value)}
            className="input-field" placeholder="Ej: 2.5, 1.5, 4" required />
        </div>
      ...
      {result && (
        <div className="space-y-6 animate-fadeInUp">
          ...
          <div className="glass-card p-6">
            <h3 className="text-lg font-semibold mb-4" style={{ color: 'var(--text-primary)' }}>📋 Detalle de Polinomios Lᵢ(x)</h3>
          ...
          <div className="glass-card p-6">
            ...
            {showChart && (
              <div className="mt-4">
                <InterpolationChart points={numericPoints} interpolantFn={result.interpolantFn} title="Polinomio de Lagrange" />
              </div>
            )}
          </div>
        </div>
