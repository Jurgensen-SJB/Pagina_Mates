...
export default function TrapezoidPage() {
  const method = getMethodById('trapezoid')!;
  const { result, loading, error, warning, execute, clear } = useCalculation('trapezoid');
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      <div className="mb-8 animate-fadeInUp">
        ...
        <div className="mt-3 p-3 rounded-lg text-sm" style={{ background: 'var(--surface)', color: 'var(--text-muted)' }}>
          <strong style={{ color: 'var(--text-secondary)' }}>Fórmula:</strong> I ≈ (h/2) · [f(x₀) + 2·Σf(xᵢ) + f(xₙ)], h = (b-a)/n
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="glass-card p-6 mb-8 animate-fadeInUp" style={{ animationDelay: '0.1s' }}>
        ...
        <div className="space-y-4">
          <FunctionInput value={funcStr} onChange={setFuncStr} onError={setFuncError} label="f(x) =" placeholder="Ej: x^2" />
          <div className="grid grid-cols-3 gap-4">
            ...
            <div>
              <label className="block text-sm font-medium mb-1" style={{ color: 'var(--text-secondary)' }}>n (subintervalos)</label>
              <input type="number" value={n} onChange={e => setN(e.target.value)} className="input-field" required min="1" max="10000" />
            </div>
          </div>
        </div>
        ...
        <div className="space-y-6 animate-fadeInUp">
          ...
          <div className="glass-card p-6">
            ...
            {showChart && parsedFn.success && parsedFn.fn && (
              <div className="mt-4">
                <IntegrationChart fn={parsedFn.fn} a={parseFloat(a)} b={parseFloat(b)} n={parseInt(n)} title="Área bajo la curva (Trapecio)" />
              </div>
            )}
          </div>
        </div>
