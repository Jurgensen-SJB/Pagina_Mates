...
export default function Simpson13Page() {
  const method = getMethodById('simpson-13')!;
  const { result, loading, error, warning, execute, clear } = useCalculation('simpson-13');
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      <div className="mb-8 animate-fadeInUp">
        ...
        <div className="mt-3 p-3 rounded-lg text-sm" style={{ background: 'var(--surface)', color: 'var(--text-muted)' }}>
          <strong style={{ color: 'var(--text-secondary)' }}>Fórmula:</strong> I ≈ (h/3) · [f(x₀) + 4·Σf(x_impar) + 2·Σf(x_par) + f(xₙ)]. Error O(h⁴). Requiere n par.
        </div>
      </div>
          ...
          <div className="grid grid-cols-3 gap-4">
            ...
            <div>
              <label className="block text-sm font-medium mb-1" style={{ color: 'var(--text-secondary)' }}>
                n (subintervalos, <span style={{ color: 'var(--warning)' }}>debe ser par</span>)
              </label>
              <input type="number" value={n} onChange={e => setN(e.target.value)} className="input-field" required min="2" max="10000" step="2" />
            </div>
          </div>
        </div>
        ...
        <div className="space-y-6 animate-fadeInUp">
          ...
          <div className="glass-card p-6">
            ...
            {showChart && parsedFn.success && parsedFn.fn && (
              <div className="mt-4">
                <IntegrationChart fn={parsedFn.fn} a={parseFloat(a)} b={parseFloat(b)} n={parseInt(n)} title="Área bajo la curva (Simpson 1/3)" />
              </div>
            )}
          </div>
        </div>
